...
def holiday_adjust(trade_date):
    ...
    if [item for item in hol.get("JP", year) if item[0].date() == trade_date]:
        ...
        holiday_adjust(trade_date)
    # if adjusted day is us holiday
    elif [item for item in hol.get("US", year) if item[0].date() == trade_date]:
        trade_date = trade_date + dt.timedelta(days=1)
        holiday_adjust(trade_date)
...
def algo_loop(total_data, trading_period=30):
    ...
    trades = pd.DataFrame(
        columns=['Signal', 'USD_IR', 'JPY_IR', 'FX_Rate', 'Equity', 'Asset Pos', 'Unreal_PnL', 'Real_PnL'],
    ...
    for index, row in total_data.iterrows():
        ...
        if current_pos == 0:
            ...
            if signal < 0:  # borrow USD and invest in JPY
                ...
                unreal_pnl = calculate_pnl(leverage=leverage, r_foreign=r_foreign, r_domestic=r_domestic,
                                           rate_open=rate_open, rate_close=spot_fx_rate)

                # record trading info
                record_trade(trade_df=trades, idx=index, signal=signal, usd_ir=usd_ir, jpy_ir=jpy_ir,
        ...
        # position > 0 or position < 0
            ...
            if holding_period >= trading_period:
                ...
                # calculate pnl
                unreal_pnl = 0
                temp_pnl = calculate_pnl(leverage=leverage, r_foreign=r_foreign, r_domestic=r_domestic,
                                         rate_open=rate_open, rate_close=spot_fx_rate)
